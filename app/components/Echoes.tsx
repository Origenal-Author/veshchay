...
export default function Echoes({ videoId, userId }: { videoId: string, userId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    // Подгружаем ники отдельно
    const ids = [...new Set(data.map(e => e.user_id))]
    const { data: profiles } = await supabase
      .from('profiles')
      .select('id, username')
      .in('id', ids)
    const map: Record<string, string> = Object.fromEntries((profiles || []).map(p => [p.id, p.username || 'аноним']))

    setEchoes(data.map(e => ({ ...e, username: map[e.user_id] || 'аноним' })))
  }, [videoId])
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true)
    const supabase = createClient()
    const { error } = await supabase.from('echoes').insert({ video_id: videoId, user_id: userId, content: text.trim() })
    if (!error) {
      setText('')
      await load()
    }
    ...
    <div style={{ marginTop: 32, borderTop: '1px solid var(--border)', paddingTop: 24 }}>
      ...
      {userId ? (
        <form onSubmit={submit} style={{ display: 'flex', gap: 10, marginBottom: 24 }}>
          ...
          />
          <button type="submit" disabled={loading || !text.trim()} className="btn-primary-ui" style={{ padding: '10px 18px', fontSize: 11, opacity: loading ? 0.6 : 1 }}>
            {loading ? '...' : 'ОТПРАВИТЬ'}
          </button>
        </form>
      ) : (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {echoes.map(echo => (
          <div key={echo.id} style={{ padding: '12px 16px', background: 'var(--surface)', border: '1px solid var(--border)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              <div style={{ width: 24, height: 24, background: 'linear-gradient(135deg,var(--accent),var(--subtext))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 9, fontWeight: 700, color: 'var(--bg)', fontFamily: "'Orbitron',monospace", flexShrink: 0 }}>
                {(echo.username || '??').slice(0, 2).toUpperCase()}
              </div>
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 11, color: 'var(--accent)' }}>@{echo.username}</span>
              <span style={{ fontFamily: "'JetBrains Mono',monospace", fontSize: 10, color: 'var(--subtext)', marginLeft: 'auto' }}>{timeAgo(echo.created_at)}</span>
              {userId === echo.user_id && (
